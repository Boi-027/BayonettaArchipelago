...
void GameHook::UpdateHooks() {
	...
	GameHook::ToggleHook(GameHook::moveIDSwap, true);
	...
	GameHook::ToggleHook(GameHook::alwaysWitchTime, true);
...
static __declspec(naked) void MoveIDSwapDetour(void) { // player in ecx
	_asm {
		pushfd
		// Player-only gate: skip both features entirely for enemies
		push eax
		mov eax, [GameHook::playerPointerAddress]
		cmp[eax], ecx
		pop eax
		jne originalcode
		// Archipelago move filter (torture/punch/kick/etc): edx is the proposed move ID
		push eax
		push ecx
		push edx
		call Archipelago::FilterMoveID
		add esp, 4
		mov edx, eax
		pop ecx
		pop eax
		// BayoHook move-ID swap feature
...
static float apWitchTimeZero = 0.0f;
static __declspec(naked) void AlwaysWitchTimeDetour(void) {
	_asm {
		cmp byte ptr[Archipelago::g_apWitchTimeLocked], 0
		je apunlocked
		movss xmm1, [apWitchTimeZero]
