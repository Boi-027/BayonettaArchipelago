...
void GameHook::GameImGui(void) {
    ...
    if (ImGui::BeginTabBar("Trainer", ImGuiTabBarFlags_FittingPolicyScroll | ImGuiTabBarFlags_NoTooltip)) {
        ...
        if (ImGui::BeginTabItem("Archipelago")) {
            float apHeight = 460.0f;
            ImGui::BeginChild("ArchipelagoChild", ImVec2(0, apHeight));
        ...
        if (ImGui::BeginTabItem("Archipelago")) {
            // Fixed height: the Archipelago sub-tabs use their own child
            // regions, which don't grow the outer cursor, so without a fixed
            // size here BayoHook's auto-sizing collapses the window to a sliver.
            float apHeight = 460.0f;
            ImGui::BeginChild("ArchipelagoChild", ImVec2(0, apHeight));
...
void GameHook::BackgroundImGui(void) {
    // Change this from DrawTab() to DrawOverlay() so it ONLY shows notifications
    Archipelago::DrawOverlay();
