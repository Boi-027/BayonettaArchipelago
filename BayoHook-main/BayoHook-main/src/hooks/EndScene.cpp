...
static void HotkeyStuff() {
	if (GameHook::hk_toggle_menu->check(GameHook::g_input)) {
...
HRESULT __stdcall Base::Hooks::EndScene(LPDIRECT3DDEVICE9 pDevice) {
	...
	if (!Data::InitImGui) {
		...
		Archipelago::InitLogoTexture(pDevice);
		Data::InitImGui = true;
	...
	ImGui::Begin("Background window", NULL, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar |
	...
	if (Data::ShowMenu) {
		ImGui::Begin(GameHook::dllName, NULL, ImGuiWindowFlags_NoResize);

		// Allow dragging the window from anywhere inside its body
