...
Vec3 WorldVisualizer::RotatePoint(const Vec3& point, const Vec3& center, const Matrix3x3& rotation) {
    Vec3 local = Vec3{ point.x - center.x, point.y - center.y, point.z - center.z };
    Vec3 rotated;
    ...
    return Vec3{ rotated.x + center.x, rotated.y + center.y, rotated.z + center.z };
}
...
void WorldVisualizer::DrawWorldSphere(const Vec3& center, float radius, ImU32 color, int segments, float thickness, const Matrix3x3* rot) {
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        Vec3 p1 = Vec3{ center.x + radius * cosf(a1), center.y + radius * sinf(a1), center.z };
        Vec3 p2 = Vec3{ center.x + radius * cosf(a2), center.y + radius * sinf(a2), center.z };
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        Vec3 p1 = Vec3{ center.x + radius * cosf(a1), center.y, center.z + radius * sinf(a1) };
        Vec3 p2 = Vec3{ center.x + radius * cosf(a2), center.y, center.z + radius * sinf(a2) };
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        Vec3 p1 = Vec3{ center.x, center.y + radius * cosf(a1), center.z + radius * sinf(a1) };
        Vec3 p2 = Vec3{ center.x, center.y + radius * cosf(a2), center.z + radius * sinf(a2) };
...
void WorldVisualizer::DrawWorldCapsule(const Vec3& start, const Vec3& end, float radius, ImU32 color, int segments, float thickness, const Matrix3x3* rot) {
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        Vec3 offset1 = Vec3{ radius * cosf(angle1), 0.0f, radius * sinf(angle1) };
        Vec3 offset2 = Vec3{ radius * cosf(angle2), 0.0f, radius * sinf(angle2) };

        // Rotate offsets if a rotation is provided
        if (rot) {
            offset1 = WorldVisualizer::RotatePoint(offset1, Vec3{ 0.0f, 0.0f, 0.0f }, rotation);
            offset2 = WorldVisualizer::RotatePoint(offset2, Vec3{ 0.0f, 0.0f, 0.0f }, rotation);
        }
